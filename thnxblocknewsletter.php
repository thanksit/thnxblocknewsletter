<?php
class thnxblocknewsletter extends Module
{
	...
	public $css_files = array(
		array(
			'key' => 'thnxblocknewsletter_css',
			'src' => 'thnxblocknewsletter.css',
			'priority' => 50,
			'media' => 'all',
			'load_theme' => false,
		),
	...
	public $js_files = array(
		array(
			'key' => 'thnxblocknewsletter_js',
			'src' => 'thnxblocknewsletter.js',
			'priority' => 50,
	...
	{
		$this->name = 'thnxblocknewsletter';
		$this->tab = 'front_office_features';
		$this->need_instance = 0;
		$this->controllers = array('verification');
		$this->bootstrap = true;
		parent::__construct();
		$this->displayName = $this->l('Platinum Theme Newsletter block');
		$this->description = $this->l('Platinum Theme Adds a block for newsletter subscription.');
		...
		$this->valid = false;
		$this->_files = array(
			'name' => array('thnxnewsletter_conf', 'thnxnewsletter_voucher'),
			'ext' => array(
				0 => 'html',
				1 => 'txt'
			)
	...
	{
		if (!parent::install() || !Configuration::updateValue('PS_NEWSLETTER_RAND', rand().rand()) || !$this->registerHook(array('header', 'displayFooterTopFullwidth', 'actionCustomerAccountAdd')))
			return false;
	...
	{
		Db::getInstance()->execute('DROP TABLE '._DB_PREFIX_.'newsletter',false);
	...
	public function displayViewCustomerLink($token = null, $id, $name = null)
	{
		...
		return $this->fetch('module:'.$this->name.'/views/templates/admin/list_action_viewcustomer.tpl');
	}
	...
	public function displayEnableLink($token, $id, $value, $active, $id_category = null, $id_product = null, $ajax = false)
	{
		...
		return $this->fetch('module:'.$this->name.'/views/templates/admin/list_action_enable.tpl');
	}
	...
	public function displayUnsubscribeLink($token = null, $id, $name = null)
	{
		...
		return $this->fetch('module:'.$this->name.'/views/templates/admin/list_action_unsubscribe.tpl');
	}
	...
	protected function unregister($email, $register_status)
	{
		...
		if (!isset($sql) || !Db::getInstance()->execute($sql,false))
			return false;
	...
	{
		return Mail::Send($this->context->language->id, 'thnxnewsletter_voucher', Mail::l('Newsletter voucher', $this->context->language->id), array('{discount}' => $code), $email, null, null, null, null, null, dirname(__FILE__).'/mails/', false, $this->context->shop->id);
	}
	...
	{
		return Mail::Send($this->context->language->id, 'thnxnewsletter_conf', Mail::l('Newsletter confirmation', $this->context->language->id), array(), pSQL($email), null, null, null, null, null, dirname(__FILE__).'/mails/', false, $this->context->shop->id);
	}
	protected function sendVerificationEmail($email, $token)
	{
		$verif_url = Context::getContext()->link->getModuleLink(
			'thnxblocknewsletter', 'verification', array(
				'token' => $token,
			)
		);
		...
		if (Tools::isSubmit('submitNewsletter'))
		{
			...
			if ($this->error)
			{
				Media::addJsDef(array(
					'msg_newsl' => $this->error,
					'nw_error' => true,
					'alert_thnxblocknewsletter' => 'Newsletter : '.$this->error,
					'placeholder_thnxblocknewsletter' => 'Enter your e-mail'
				));
				$this->smarty->assign(
					array(
						'color' => 'red',
			...
			{
				Media::addJsDef(array(
					'msg_newsl' => $this->valid,
					'nw_error' => false,
					'alert_thnxblocknewsletter' => 'Newsletter : '.$this->valid,
					'placeholder_thnxblocknewsletter' => 'Enter your e-mail'
				));
				$this->smarty->assign(
					array(
	...
	public function hookFooter($params)
	{
		...
		$this->prepared = true;
		return $this->fetch('module:'.$this->name.'/views/templates/hook/thnxblocknewsletterfooter.tpl');
	}
	...
	public function hookdisplayFooterTopFullwidth($params)
	{
		if (!isset($this->prepared) || !$this->prepared)
			$this->_prepareHook($params);
		$this->prepared = true;
		return $this->fetch('module:'.$this->name.'/views/templates/hook/thnxblocknewsletter.tpl');
	}
	public function hookdisplayhomefullwidthmiddle($params)
	{
		return $this->hookdisplayFooterTopFullwidth($params);
	}
	...
	public function hookActionCustomerAccountAdd($params)
	{
		...
		if (Validate::isEmail($email))
			return (bool)Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'newsletter WHERE id_shop='.(int)$id_shop.' AND email=\''.pSQL($email)."'",false);
	...
	public function export_csv()
	{
		...
		if ($result)
		{
			...
			elseif ($fd = @fopen(dirname(__FILE__).'/'.strval(preg_replace('#\.{2,}#', '.', Tools::getValue('action'))).'_'.$this->file, 'w'))
			{
				...
				<a href="'.$this->context->shop->getBaseURI().'modules/thnxblocknewsletter/'.Tools::safeOutput(strval(Tools::getValue('action'))).'_'.$this->file.'">
